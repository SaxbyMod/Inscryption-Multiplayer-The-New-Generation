...
# fonts
nameFont = ImageFont.truetype('Cambria.ttf', 50)
statFont = ImageFont.truetype('Cambria.ttf', 74)
textFont = ImageFont.truetype('Cambria.ttf', 40)
boldFont = ImageFont.truetype('Cambria.ttf', 50)
italFont = ImageFont.truetype('Cambria.ttf', 45)
artsFont = ImageFont.truetype('Cambria.ttf', 42)
formatFont = ImageFont.truetype('Cambria.ttf', 20)
tribesFont = ImageFont.truetype('Cambria.ttf', 42)
...
def printCard(info,savePath="output",show=False,prefix="01x 001 ",fmt=""):
    ...
    img = Image.new("RGBA",(120,177))
    
    # The background colors.
    bgPath = "bg/bg_"
    if info["tier"]=="Rare":
        bgPath += "rare_"
    elif info["tier"]=="Common":
        bgPath += "common_"
    ...
    # end try
    img.paste(artImg,(17,16),artImg.convert("RGBA"))
    artImg.close()
    ...
    # Scale it up to fit.
    img = img.resize((1200,1770),resample=Image.Dither.NONE)
            ...
            if i[0] < threshold:
                for j in range(i[0]):
                    img.paste(costImg,
                              ...
                              costImg.convert("RGBA"), anchor = "ra"
                              )
                    costx -= (w-1)
                costx -= 2
            else:
                ...
                costx -= ((w-1)+6)
                alphaPaste(img,costx-45,costy-15,"cost/x.png")
                costx-=6
                for j in range(len(numberString)):
                    chrj = numberString[-j-1]
                    symbolString = "cost/"+chrj+".png"
                    alphaPaste(img,costx-75,costy-15,symbolString)
                    costx -= 6
                # end for
                costx += (w-4)
            # end if
    ...
    if True:
        ...
        normalPower = True
        if info["sigils"] != []:
            for i in info["sigils"]:
                if "Power" in i:
                    normalPower = False
                    try:
                        alphaPaste(img,45,1644,"sigils/variable/"+i.strip()+".png")
                    except FileNotFoundError:
                        print("unknown variable power: "+i)
                        shadowText(I1,75,1644,str(int(info["power"])),
                                   statFont,anchor="ma")
                    # end try
                # end if
            # end for
        # end if
        if normalPower:
            shadowText(I1,75,1644,str(int(info["power"])),statFont,anchor="ma")
        # end if
    # end if
    
    shadowText(I1,1125,1644,str(int(info["health"])),statFont,anchor="ma")
    ...
    for isig in info["sigils"]:
        ...
        I1.text((sigilx, sigily+textOffset-145),
                isig,
                fill=(0,0,0),
                font=boldFont,
                anchor="ma"
        ...
        I1.text((sigilx, sigily+textOffset-2),
                textLines[0],
                fill=(0,0,0),
                font=textFont,
                anchor="ma"
        ...
        if singleLine == False:
            # skip the first one
            for i in range(n)[1:]:
                I1.text((sigilx, sigily+textOffset+40*i-2),
                textLines[i],
                fill=(0,0,0),
                font=textFont,
                anchor="ma"
    ...
    # Name the card.
    shadowText(I1,600,30,info["name"],nameFont,anchor="ma")

    # Do not let us forget what this is for in the end.
    if fmt != "":
        print(info["artist"]+" - "+fmt)
        tx,ty = FORMATPOS
        I1.text(FORMATPOS,info["artist"]+" - "+fmt,fill=(0,0,0),font=formatFont,anchor="ma")
    #end if
